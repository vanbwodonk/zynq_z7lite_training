...
module test_card_image #(
    ...
    localparam CENTER_HR = H_RES / 2;
    localparam CENTER_VR = V_RES / 2;
    localparam HALF_IMG_H_RES = IMAGE_H_RES / 2; 
    localparam HALF_IMG_V_RES = IMAGE_V_RES / 2; 
    localparam OFFSET_X = CENTER_HR - HALF_IMG_H_RES;
    localparam OFFSET_Y = CENTER_VR - HALF_IMG_V_RES;
    localparam MAX_X = CENTER_HR + HALF_IMG_H_RES;
    localparam MAX_Y = CENTER_VR + HALF_IMG_V_RES;
    localparam STOP_ADDR = IMAGE_H_RES * IMAGE_V_RES - 1;
    localparam ROM_DELAY = 2;

    reg [15:0] rd_addr,rd_addr_old;
    reg [1:0]   delay_cnt;
    wire [23:0] dprocess;
    reg	[23:0]  dout;
    reg addr_change_detect;
    ...
    always @(posedge clk) begin       
        ...
        else if((i_x >= OFFSET_X) && (i_x < MAX_X) && (i_y >= OFFSET_Y) && (i_y < MAX_Y)) begin
    ...
    always @(i_x or i_y) 
    begin 
         if((i_x > OFFSET_X) && (i_x <= MAX_X) && (i_y > OFFSET_Y) && (i_y <= MAX_Y)) 
            dout <= dprocess;
         else 
            dout <= 24'hfee101;
    end
    ...
    rom_image inst_rom (
      ...
      .douta(dprocess)  // output wire [23 : 0] douta
    );

    // Colour Output
    assign o_blue    = dout[7:0];
    assign o_green  = dout[15:8];
    assign o_red   = dout[23:16];
endmodule
